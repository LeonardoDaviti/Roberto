...
def story_claims_from_story(
    *,
    ...
    evidence = dedupe_source_refs([ref.as_ref_dict() for ref in story.source_refs])
    if not evidence and story.sources:
        evidence = dedupe_source_refs([x_source_ref(username=s.username, tweet_id=s.tweet_id) for s in story.sources])
